...
public class QueueAC<T> implements Queue<T> { 
     ...
     *  in a cyclic way (modulo theArray.length)
     */
    ...
    // Methods that change the queue's state:
    ...
    public T dequeue() {
        ...
        T e = theArray[first];
        theArray[first] = null; // Optional: help garbage collection
        first = increase(first);
        size--;
        return e;
    } 

    // Methods that query the queue's state:
